...
4. Finns Jupyter interactive notebooks Coding style: Finn uses the jupyter interactive program on top of .py files. So code should be wrapped in #%%'s to allow for easy running and testing of code blocks. The code should be structured in a way so that files are used to split major sections of code so general legnth of each file is less than 300 lines (max 500 lines). The workflow should be to have a central .py file, e.g. transport_leap_import.py which will utilise its own functions and classes, as well as importing functions and classes from other files, e.g. transport_measures.py, transport_measure_metadata.py, transport_branch_mappings.py etc. and the workflow should be at the bottom of the file so its easy to find and run. Transport measure helpers have now been separated into transport_measure_catalog.py, transport_measure_processing.py, and transport_preprocessing.py for clarity.
