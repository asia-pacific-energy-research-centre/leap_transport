...
def _get_scalar(
    ...
    matched_series = None
    for candidate_path in transport_branch_path_candidates(path):
        series = df.loc[(df["Branch Path"] == candidate_path) & (df["Variable"] == variable), base_year]
        if not series.empty:
    ...
    if len(matched_series) > 1:
        raise ValueError(f"Expected exactly one value for {variable} at {path}, found {len(matched_series)}")
    value = float(matched_series.iloc[0])
    if pd.isna(value):
        if allow_missing:
            return default if default is not None else 0.0
...
def transport_adjustment_fn(
    ...
    if strategy == "Stock":
        ...
        #   device_path = Demand\Transport\Passenger road\Buses\Gas and diesel oil
        ...
        # 2) Scale road-efficiency terms for THIS device branch by f.
        # Keep "Final On-Road Fuel Economy" in sync with "Fuel Economy" so
        # LEAP correction factors are not implicitly altered by reconciliation.
        for variable in ("Mileage", "Fuel Economy", "Final On-Road Fuel Economy"):
        ...
        if is_non_road_transport_branch_path(branch_path) and branch_tuple_depth(branch_path) >= 3:
...
def transport_intensity_energy_fn(
    ...
    if logical_depth <= 2:
        ...
        # Leaf activity is encoded as a single share (%) at the leaf node itself.
        activity_level_share1 = _get_scalar(export_df, base_year, branch_path, "Activity Level")
        ...
        if not is_non_road_transport_branch_path(branch_path):
            raise ValueError(
    ...
    if is_pipeline_or_nonspecified_branch_path(branch_path):
        # For Others branches, intensity starts at 1 in input creation but may be
        # scaled during reconciliation; treat it as already in compatible units.
        intensity_scale = 1.0
...
def verify_nonroad_intensity_leaf_adjustment(
    ...
    print(f"Energy adjustment multiplier: {energy_scale_factor}")
...
def build_transport_esto_energy_totals(
    ...
    for esto_key, leap_branches in NINTH_SOURCE_TO_LEAP_BRANCH_MAP.items():
        ...
        # leap_branches is a list of tuples like ('Passenger road', 'Cars', 'ICE', 'Motor gasoline')
        ...
        if has_nonspecified:
            ...
            try:
                ...
                if tmp_base.empty:
                    print(
                        f"ℹ️  No nonspecified ESTO rows for key {esto_key} at {base_year}; "
                        "marking as missing and skipping reconciliation for this key."
                    )
                    esto_energy_totals[esto_key] = float("nan")
                else:
        ...
        if len(esto_key) == 3:
            ...
            raise ValueError(
                f"Unsupported ESTO key length for {esto_key}. Expected 3 or 4 elements."
            )

        mask = (
            (esto_filtered["sub1sectors"] == sub1sector)
            & (esto_filtered["sub2sectors"] == sub2sector)
            & (esto_filtered["fuels"] == fuel)
            & (esto_filtered["subfuels"] == subfuel)
        )
        ...
        if len(esto_rows) == 1:
            ...
            print(
                ...
                f"for {base_year}. Marking as missing and skipping reconciliation for this key."
            )
            esto_energy_totals[esto_key] = float("nan")
        else:
            # Some datasets carry multiple rows for the same ESTO key (e.g., with
            # deeper sector detail). Aggregate them for reconciliation.
            summed_total = pd.to_numeric(esto_rows[base_year], errors="coerce").sum(min_count=1)
            print(
                f"ℹ️  Multiple ESTO rows found for key {esto_key} in {economy}/{original_scenario} "
                f"for {base_year}; summing {len(esto_rows)} rows."
            )
            esto_energy_totals[esto_key] = float(summed_total)
