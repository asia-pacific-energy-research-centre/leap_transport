...
def extract_other_type_rows_from_esto_and_insert_into_transport_df(df, base_year, final_year, economy,scenario, TRANSPORT_ESTO_BALANCES_PATH = 'data/merged_file_energy_ALL_20251106.csv'):
    ...
    if len(other_leap_branches) > 0:
        other_rows = extract_esto_energy_use_for_leap_branches(other_leap_branches, esto_energy_use, economy, scenario, base_year, final_year)
...
def extract_esto_energy_use_for_leap_branches(leap_branches, esto_energy_use,economy, scenario, base_year=2022, final_year=2060):
    ...
    for leap_branch, esto_rows in esto_sector_fuels_for_other.items():
        ...
        for esto_key in esto_rows:
            if len(esto_key) == 3:
                subsector, fuel, subfuel = esto_key
                sub2sector = 'x'
            elif len(esto_key) == 4:
                subsector, sub2sector, fuel, subfuel = esto_key
            else:
                raise ValueError(f"Unsupported ESTO key length in extraction: {esto_key}")
            ...
            esto_row_base_year = esto_energy_use[
                ...
                (esto_energy_use['sub2sectors'] == sub2sector) &
        ...
        # if subsector =='15_05_pipeline_transport':
        #     breakpoint()#why is it becming so high? i think acitivty level is being set super high somehow?
